...
class BbsController extends EdsController {
    ...
    public function ex_trend_edit($zid = -1){
        ...
        $rec = $m->where($lim)->find();
        if($rec){
            if($rec['ztype'] == 6 && $rec['zcreator'] == session('rid')){
                $this->zid = $zid;
                $this->ztitle = $rec['ztitle'];
    ...
    public function ex_trend_save($zid=-1,$ztitle='',$zsummary='',$zcontent='',$zrelease=''){
        if(session('g_logined')!='logined'){//未登录
            $this->show('{"result":1,"msg":"validation error."}', 'utf-8');
            return;
        }
    ...
    public function ex_del($zid=''){
        ...
        $lim['zid'] = $zid;
        $rec = $m->where($lim)->find();
        if($rec && ($rec['ztype']==5||$rec['ztype']==6||$rec['ztype']==9) && $rec['zcreator']==session('rid') && $rec['zstate']!=60){
            $data['zid'] = $zid;
            $data['zstate'] = 60;
            if($m->save($data)){
                $this->show('{"result":0,"msg":"saving succeed."}', 'utf-8');
